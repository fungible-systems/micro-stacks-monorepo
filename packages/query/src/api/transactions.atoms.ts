...
>(
  ...
  async function queryFn(_get, [url, limit = DEFAULT_LIST_LIMIT, type], { pageParam: offset = 0 }) {
...
>(
  ...
  async function queryFn(_get, [url, limit = DEFAULT_LIST_LIMIT], { pageParam: offset = 0 }) {
...
>(TxClientKeys.Transaction, async function queryFn(_get, [url, txid]) {
...
export const rawTransactionClient = atomFamilyWithQuery<[networkUrl: string, txid: string], string>(
  ...
  async function queryFn(_get, [url, txid]) {
